...
const Typography = ({ size = "medium", weight = "regular", color = "#333", children, }) => {
    loadGoogleFont();
    return (jsx("span", { style: Object.assign(Object.assign({}, typographyStyles), { fontSize: fontSizeMap[size], fontWeight: fontWeightMap[weight], color: color }), children: children }));
};
...
const getIcon = (type) => {
    ...
    switch (type) {
        case "text":
            return jsx(FaUser, { style: iconStyle });
        case "password":
            return jsx(FaLock, { style: iconStyle });
        case "email":
            return jsx(FaEnvelope, { style: iconStyle });
        case "number":
            return jsx(FaHashtag, { style: iconStyle });
        case "date":
            return jsx(FaCalendar, { style: iconStyle });
        case "textarea":
            return jsx(FaComment, { style: iconStyle });
        default:
            return null;
    }
};
const Input = ({ type, label, placeholder, value, options, checked, size = "medium", width, height, onChange }) => {
    ...
    return (jsxs("div", { style: { display: "flex", flexDirection: "column", marginBottom: "15px", position: "relative", maxWidth: "100%" }, children: [label && jsx("label", { style: { fontSize: "14px", fontWeight: 600, marginBottom: "5px", color: "#333" }, children: label }), jsxs("div", { style: { display: "flex", alignItems: "center", position: "relative", width: "100%" }, children: [getIcon(type), type === "textarea" ? (jsx("textarea", { style: inputStyles, placeholder: placeholder, value: value, onChange: onChange })) : type === "select" && options ? (jsx("select", { style: inputStyles, value: value, onChange: onChange, children: options.map((option, index) => (jsx("option", { value: option, children: option }, index))) })) : (jsx("input", { style: inputStyles, type: type, placeholder: placeholder, value: value, checked: checked, onChange: onChange }))] })] }));
};

const Table = ({ data, columns, sortColumn, sortDirection, onSort, width }) => {
    return (jsx("div", { style: { maxWidth: width || "800px", fontFamily: "'Poppins', sans-serif", overflowX: "auto" }, children: jsxs("table", { style: { width: "100%", borderCollapse: "collapse", marginTop: "20px" }, children: [jsx("thead", { children: jsx("tr", { style: { background: "#f4f4f4", textAlign: "left" }, children: columns.map((col) => (jsxs("th", { style: { padding: "12px", borderBottom: "2px solid #ddd", cursor: col.sortable ? "pointer" : "default" }, onClick: () => col.sortable && onSort(col.accessor), children: [col.header, " ", col.sortable &&
                                    (sortColumn === col.accessor ? (sortDirection === "asc" ? jsx(FaSortUp, {}) : jsx(FaSortDown, {})) : jsx(FaSort, {}))] }, col.accessor))) }) }), jsx("tbody", { children: data.map((row, index) => (jsx("tr", { style: { background: index % 2 === 0 ? "#fff" : "#f9f9f9" }, children: columns.map((col) => (jsx("td", { style: { padding: "12px", borderBottom: "1px solid #ddd" }, children: row[col.accessor] }, col.accessor))) }, index))) })] }) }));
};
...
class ListBox extends Component {
    ...
    render() {
        ...
        return (jsx("div", { style: {
            ...
            }, children: options.map((option) => (jsxs("label", { style: {
                    display: "flex",
                    alignItems: "center",
                    cursor: "pointer",
                    gap: "5px",
                }, children: [jsx("input", { type: "radio", name: "listbox", value: option, checked: selected === option, onChange: () => this.handleSelect(option) }), option] }, option))) }));
    }
}

// Header Component
const Header = ({ children, height = "60px" }) => (jsx("div", { style: { height, background: "#333", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "18px", fontWeight: "bold" }, children: children }));
// Footer Component
const Footer = ({ children, height = "50px" }) => (jsx("div", { style: { height, background: "#222", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "16px" }, children: children }));
const Sidebar = ({ children, width = "250px" }) => (jsx("div", { style: { width, background: "#f8f9fa", height: "100vh", padding: "20px", boxShadow: "2px 0 5px rgba(0,0,0,0.1)" }, children: children }));
const MainContent = ({ children, headerHeight = 60, footerHeight = 50 }) => {
    const contentHeight = `calc(100vh - ${headerHeight}px - ${footerHeight}px)`;
    return (jsx("div", { style: { flex: 1, padding: "20px", overflowY: "auto", height: contentHeight }, children: children }));
};

export { Button, Footer, Header, Input, ListBox, MainContent, Sidebar, Table, Typography };
